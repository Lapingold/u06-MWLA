...
class Tab2Controller:
    ...
    def __init__(self, tab_parent):
        ...
        self.tab = ttk.Frame(tab_parent)
        tab_parent.add(self.tab, text="View all users")

        self.allLabelTab2 = tk.Label(self.tab, text="Users")
        self.imgLabelTab2 = tk.Label(self.tab)

        self.buttonCommit = tk.Button(self.tab, text="Users", command=self.all_users, activebackground="yellow")
        # === ADD WIDGETS TO GRID ON TAB two
        self.allLabelTab2.grid(row=1, column=0, padx=15, pady=15)
        self.imgLabelTab2.grid(row=1, column=2, rowspan=3, padx=15, pady=15)
        self.buttonCommit.grid(row=5, column=1, padx=15, pady=15)
    ...
    def all_users(self):
        users = database.get_user(True)
        pl_select = tk.Listbox(self.tab, height=10, width=40)
        pl_select.grid(row=5, column=2, padx=10, pady=30)
        result = []
        for item in users:
        ...
        text_scroll = tk.Scrollbar(self.tab, orient="vertical", command=pl_select.yview)
        pl_select["yscrollcommand"] = text_scroll.set
        text_scroll.grid(row=5, column=3, sticky="ns")
